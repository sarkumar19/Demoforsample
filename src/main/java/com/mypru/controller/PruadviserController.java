...
public class PruadviserController {
	@Autowired
	private PruadviserService pruadviserservice;
	...
	{ 
		...
		{
			logger.error("the pruadviser details are not found",e.getMessage());
			throw new ResponseStatusException(HttpStatus.FOUND,"Pruadviser details not found",e);
		}
	}
	...
	public ResponseEntity<?> getPruadviserDetailsByName(@PathVariable String pruadviserName)
	{
		...
		{
			logger.error("The requested name not found ",e.getMessage());
	...
	public ResponseEntity<?> getPruadviserDetailsById(@PathVariable Long id)
	{
		...
		return ResponseEntity.ok(pruadviser);
		 ...
		 {
			 logger.error("the pruadviser details are not found" +id,e);
			 return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
